...
//
// Fixtures are written in metres and converted with PIXELS_PER_METER, so the solver
// is tested against the same house whatever the drawing scale is.
//
...
check('the camera lands inside the room', one.cameras[0].x > 0 && one.cameras[0].x < m(6) && one.cameras[0].y > 0 && one.cameras[0].y < m(6), JSON.stringify(one.cameras[0]))
check('the camera is kept off the walls', Math.min(one.cameras[0].x, m(6) - one.cameras[0].x, one.cameras[0].y, m(6) - one.cameras[0].y) > m(0.2), JSON.stringify(one.cameras[0]))
...
check('the camera sits in a corner, not the middle', Math.min(one.cameras[0].x, m(6) - one.cameras[0].x) < m(1) || Math.min(one.cameras[0].y, m(6) - one.cameras[0].y) < m(1), JSON.stringify(one.cameras[0]))
...
check('one of them is in each room', two.cameras.filter((c) => c.x < m(7.5)).length === 1 && two.cameras.filter((c) => c.x > m(7.5)).length === 1, JSON.stringify(two.cameras.map((c) => c.x)))
check('a wall is never seen through', visibleTargets({ ...two.cameras[0], room: hall }, [{ x: m(12.5), y: m(2.5), weight: 1, kind: 'floor' }], [hall, kitchen], []).length === 0)
...
const bigRoomNoDoor = rect(m(10), 0, m(7.5), m(7.5), 'Big')
const oneCamera = planCameraPlacement({ walls: [hall, bigRoomNoDoor], objects: [frontDoor], maxCameras: 1 })
check('with one camera, the room with the door wins', oneCamera.cameras[0].x < m(7.5), JSON.stringify(oneCamera.cameras[0]))
...
console.log('\nA long room')
const corridor = rect(0, 0, m(20), m(4), 'Corridor')
...
check('a half-covered plan only gets the missing room', partly.cameras.length === 1 && partly.cameras[0].x > m(7.5), JSON.stringify(partly.cameras))
...
const centred = planCameraPlacement({ walls: [hall], cameras: [{ x: m(3), y: m(3), rotation: 0, hFov: 110, distance: 12 }] })
...
console.log('\nLimits')
const manyRooms = Array.from({ length: 12 }, (_, i) => rect(i * m(7.5), 0, m(6), m(6), `Room ${i}`))
...
check('the cap still spreads the cameras out', new Set(capped.cameras.map((c) => Math.round(c.x / m(7.5)))).size === 3, JSON.stringify(capped.cameras.map((c) => c.x)))
...
check('an open wall is not a room', roomsOf([{ id: 'x', closed: false, points: [{ x: 0, y: 0 }, { x: m(9), y: 0 }, { x: m(9), y: m(9) }] }]).length === 0)
...
const proposed = { x: m(0.75), y: m(0.75), rotation: 0, hFov: 110, distance: 8, why: 'looking across the hall' }
const withProposal = planCameraPlacement({ walls: [hall], proposals: [proposed] })
check('a proposed position is available to the solver', buildCandidates([hall], [proposed]).some((c) => Math.hypot(c.x - proposed.x, c.y - proposed.y) < m(0.05)), JSON.stringify(buildCandidates([hall], [proposed]).map((c) => [Math.round(c.x), Math.round(c.y)])))
check('an equally good proposal is the one used', Math.hypot(withProposal.cameras[0].x - proposed.x, withProposal.cameras[0].y - proposed.y) < m(0.5), JSON.stringify(withProposal.cameras[0]))
check('a proposal that covers less does not win', (() => {
  // Mid-way down the room, facing nothing in particular: the corner beats it.
  const worse = planCameraPlacement({ walls: [hall, kitchen], proposals: [{ x: m(3), y: m(3), rotation: 0, hFov: 110, distance: 2 }] })
  return worse.cameras.every((c) => Math.hypot(c.x - m(3), c.y - m(3)) > m(0.5))
})())
check('a proposal outside every room is ignored', buildCandidates([hall], [{ x: m(-22.5), y: m(-22.5) }]).every((c) => c.x >= 0 && c.x <= m(6)))
